...
export const uploadDirectory = async ({ endpoint, token, path, maxConcurrentUploads, maxTimeout = 60000 }) => {
  ...
  // curry the functions to keep state around w/o passing it around
  const viewModel = partial(getViewModel, [files.length, startTime]);
  ...
  for (const file of files) {
    ...
    const logData = (newTimeout) => {
      console.clear();
      console.table(viewModel(newTimeout, filesFinished, file));
