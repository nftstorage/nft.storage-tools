...
export const uploadDirectory = async ({ endpoint, token, path, maxConcurrentUploads, maxTimeout = 60000 }) => {
  ...
  for (const fileName of files) {
    ...
    store(timeBetweenCalls, fileProps).then(processData).finally(release);
  }
};
...
const retryClientStore = async (client, maxTimeout, timeToWait, fileProps) => {
  try {
    ...
    if (timeToWait > maxTimeout) timeToWait = maxTimeout * (1 + Math.random()); //stagger the time between retries
