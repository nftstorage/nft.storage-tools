import { map, path, isEmpty,mapObjIndexed } from "ramda"
...
const getFiles = mapObjIndexed((value,key)=>{
  console.log({value,key})
  return path(["files",key])
})

export const flatten = (directory, currentDirectoryList={}) => {
  console.log({directory,currentDirectoryList})
  if(isEmpty(directory.files)) return currentDirectoryList
  return {...flatten, values:flatten(directory.files,currentDirectoryList)}
}
